<template>
  <main class="flex-grow container mx-auto px-6 py-12">
    <div class="text-center mb-10">
      <h1 class="text-4xl font-extrabold text-gray-900 dark:text-white">Həkimlərimiz</h1>
      <p class="mt-3 text-gray-500 dark:text-gray-400">Sağlamlığınız üçün ən yaxşı mütəxəssisləri tapın.</p>
    </div>

    <div class="mb-8 p-4 bg-card-light dark:bg-card-dark rounded-xl shadow-md grid md:grid-cols-3 gap-4">
      <input v-model="search" placeholder="Ad, soyad və ya ixtisas..." class="input pl-10" />
      <select v-model="specialization" class="input_doctors">
    ...
    <div v-if="loading" class="text-center text-gray-500 dark:text-gray-400 mt-10">Yüklənir...</div>
    <div v-else-if="!doctors.length" class="text-center text-gray-500 dark:text-gray-400 mt-10">Həkim tapılmadı.</div>
...
const fetchDoctors = async () => {
  ...
  try {
    const { data } = await useServer().GET("/api/doctors/", { params: { query } })
    doctors.value = data ?? []
  } catch {
    doctors.value = []
  } finally {
